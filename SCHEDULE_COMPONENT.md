...
### 4. Create a Home Assistant Schedule
...
// Get data sensor value
float temp = id(target_temp).state;
...
sensor:
  ...
  - platform: template
    ...
    lambda: 'return id(heating_temp).state;'
    ...
    heat_action:
      - lambda: |-
          ...
          if (id(heating_schedule).state) {
            ...
            auto target = id(heating_temp).state;
            if (!isnan(target)) {
    ...
    then:
      - lambda: |-
          ...
          if (id(living_room_schedule).state) {
            ...
            float target = id(living_room_temp).state;
            if (!isnan(target)) {
          ...
          if (id(bedroom_schedule).state) {
            ...
            float target = id(bedroom_temp).state;
            if (!isnan(target)) {
...
// Get data sensor value
float position = id(blinds_position).state;
